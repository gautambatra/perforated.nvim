...
3. **Annotate (`:P4 annotate`, `b`):** one `annotate -c -i -u -q [-I]` call (`-i` follows branches, `-u` gives user and date per line; the file's `filelog -l -i` is fetched only for `~`/`d`, and for descriptions by the blame line). Shown in a left scrollbound, cursorbind split with columns for CL, user and date, coloured by age with a configurable gradient. `<CR>` describes the CL, and `~` re-annotates at the revision before this line's change. Rendered with one `set_lines` call.
...
> **Status (2026-09-26): done**, except the README GIFs (to be recorded by hand).
> - Slider: a two-line window above the time-lapse buffer (not a float: it never hides the
>   first lines), click to jump; modes single / incremental diff / range (`m`, `H`/`L` for ◆).
> - p4vc: `gR` / `<C-S-r>` revision graph, P4V time-lapse in the menu, `:P4 p4vc …`, health.
> - LSP code actions: in-process server, opt-in (`lsp.enabled`), client-side commands.
> - Docs: `doc/perforated.txt` generated by `make doc` (a test fails when stale);
>   `docs/migrating.md`; `:P4 reopen` added for parity.
> - Performance: `:P4 debug timings`; memory soak test (1000 open/close cycles; asserts no
>   plugin table grows). It found three leaks (poll's BufEnter throttle table, fstat cache
>   entries for closed files and for reverted files), all fixed.
